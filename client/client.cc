...
private:
    ...
    std::string GetFromLeader(const std::string& leader_address, const std::string& key) {
      KvsClient leader_client( grpc::CreateChannel(
          leader_address, grpc::InsecureChannelCredentials() ) );
      return leader_client.Get( key );
    }

    std::string PutToLeader(const std::string& leader_address, const std::string& key, const std::string& value) {
      KvsClient leader_client( grpc::CreateChannel(
          leader_address, grpc::InsecureChannelCredentials() ) );
      return leader_client.Put( key, value );
    }

    std::string DeleteFromLeader(const std::string& leader_address, const std::string& key) {
      KvsClient leader_client( grpc::CreateChannel(
          leader_address, grpc::InsecureChannelCredentials() ) );
...
int main(int argc, char** argv) {
     if (argc < 4) {
        std::cerr << "Usage: " << argv[0] << " <target_address> <operation> <key> [value]" << std::endl;
        return 1;
    }
    ...
    if (operation == "put" && argc < 5) {
        std::cerr << "Usage for put operation: " << argv[0] << " <target_address> put <key> <value>" << std::endl;
        return 1;
    }
    ...
    KvsClient client(grpc::CreateChannel(target_address, grpc::InsecureChannelCredentials()));
