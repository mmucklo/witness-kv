...
constexpr int BLOCK_SIZE = 4096; // TODO(mmucklo): maybe make this flag-tunable.
...
void verifyFilename(std::string filename) {
  ...
  // TODO(mmucklo): maybe convert these into LOG(FATAL)s so more information about the state can be outputted?
  CHECK(!std::filesystem::exists(std::filesystem::status(filename))) << "file should not exist already.";
  std::filesystem::path path{filename};
  CHECK(path.has_parent_path()) << "file should have a parent path.";
  std::filesystem::file_status path_status = std::filesystem::status(path.parent_path());
  CHECK(std::filesystem::is_directory(path_status)) << "parent path should be a directory.";
...
FileWriter::FileWriter( std::string filename ) : filename_( std::move( filename ) ), buffer_size_(0), bytes_written_(0) {
  ...
  fd_ = open(filename_.c_str(), O_APPEND|O_CREAT|O_WRONLY, S_IRUSR|S_IWUSR);
  if (fd_ == -1) {
    LOG(FATAL) << "Could not open file descriptor for " << filename_ << " errno: " << errno << " " << strerror(errno);
  }
...
FileWriter::~FileWriter() {
  ...
  if (close(fd_) == -1) {
    LOG(FATAL) << "Error closing fd: " << fd_ << " for filename " << filename_ << " errno: " << errno << " " << strerror(errno);
  }
}
...
void FileWriter::Write(const absl::Cord& msg)
{
  ...
  for (absl::string_view chunk : msg.Chunks()) {
    ...
    while (chunk_idx < chunk_size) {
      ...
      // We should never overwrite the buffer - if so, it's certainly a crashable event.
...
void FileWriter::Flush()
{
  ...
  if (fdatasync(fd_) == -1) {
    LOG(ERROR) << "fdatasync returned -1, errno: " << errno << ": " << strerror(errno) << ", filename: " << filename_;
  }
}
