...
class LogReader {
 ...
 private:
  ...
  absl::StatusOr<std::unique_ptr<char[]>> ReadBufferLocked(uint64_t size, uint32_t crc32_val);
  ...
  // Technically we could just mark this class as non-threadsafe, but with the iterator
  // paradigm you could still have two iterators trying to read through the log at the
  // same time. Guarding all the state by the lock is safer in that respect just
  // in case we want to respect the standard notion of being able to safely
  // read from an iterator without thinking about concurrency, and more importantly
  // be able to have two separate iterators open at the same time, even if by accident
  // (e.g. one was just not destructed yet).
